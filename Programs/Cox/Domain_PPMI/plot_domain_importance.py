...
Output
    Results/Cox/Domain_PPMI/domain_cindex_ppmi.png
    Results/Cox/Domain_PPMI/feature_hr_heatmap.png
"""
...
from ppmi_feature_map import DOMAINS, INVERT_FEATURES
...
def load_domain_summary() -> pd.DataFrame:
    ...
    if not path.exists():
        raise FileNotFoundError(f"Run run_ppmi_multivariable.py first.\n{path}")
...
def load_univariate_results() -> pd.DataFrame:
    ...
    if not path.exists():
        raise FileNotFoundError(f"Run run_ppmi_univariate.py first.\n{path}")
    return pd.read_csv(path)
...
def plot_domain_cindex(summary_df: pd.DataFrame) -> None:
    ...
    out = OUTPUT_DIR / "domain_cindex_ppmi.png"
